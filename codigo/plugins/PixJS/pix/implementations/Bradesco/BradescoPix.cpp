...
std::string BradescoPix::GetToken(Pix::RawData& pd) {
       ...
       GetUnixTime() >= tokens[pd.key].expiredAt)
    {
        ...
        auto response = Request("https://qrpix-h.bradesco.com.br/oauth/token")
...
Request BradescoPix::GetRequest(Pix::RawData& pd){
    std::string url =
        std::string("https://qrpix-h.bradesco.com.br/v2/cob/") +
        pd.seq;

    auto token = BradescoPix::GetToken(pd);
...
Pix::Result BradescoPix::ProcessRequest(Request& request) {
...
Pix::Result BradescoPix::Generate(Pix::RawData pd) {
    try {
...
Pix::Information BradescoPix::Get(Pix::RawData pd) {
...
void BradescoPix::Edit(Pix::RawData pd) {
    try {
        GetRequest(pd)
...
Pix::Payment BradescoPix::IsPaid(Pix::RawData pd) {
...
std::unordered_map<std::string, BradescoPix::Token> BradescoPix::tokens = std::unordered_map<std::string, BradescoPix::Token>();