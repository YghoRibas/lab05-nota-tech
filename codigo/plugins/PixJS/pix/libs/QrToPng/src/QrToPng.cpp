...
QrToPng::QrToPng(int imgSize, int minModulePixelSize, std::string text,
                 qrcodegen::QrCode::Ecc ecc) :
        _fileName("__.png"), _size(imgSize), _minModulePixelSize(minModulePixelSize), _text(std::move(text)),
        _overwriteExistingFile(true), _ecc(ecc) {
}
...
bool QrToPng::_writeTo(const qrcodegen::QrCode &qrData, std::ostream& out) const {
    ...
    if (qrSizeFitsInMaxImgSizeTimes < _minModulePixelSize)
        return false; // image would be too small to scan
    ...
    return fs::exists(_fileName) || _fileName == "__.png";
}
